...
#    def __init__(self, maxLen = 5000, reTrainThres = 500):
    ...
    def train(self, dispData, fluidPrevData, fluidData, input_u=None, kernel='thin_plate_pline', smoothing=9.5e-2,
        ...
        self.romLoad.decompose(fluidData, rank = rank_pres, normalize = True, center = True, to_copy=False, alg="svd")
        ...
        if kernel == "lasso":
            ...
            self.func = RBFInterpolator(input_.copy(), output_.copy(), kernel = kernel,smoothing=smoothing, neighbors=neighbors)

    def augmentData(self, newdispData, newfluidPrevData, newfluidData, current_t=-1):
        dispCoeff = self.podDispLoad.project(self.romDispLoad.normalize(self.romDispLoad.center(np.vstack((newfluidPrevData, newdispData)))))
    ...
    def predict(self, newDisp, newPrevLoad, input_u=None ):
        ...
        inp_ = self.romDispLoad.normalize(self.romDispLoad.center(np.vstack((newPrevLoad, newDisp))))
        xTest = self.podDispLoad.project(inp_)

        #xTest = self.scalerIn.transform(xTest.T).T
        ...
        if self.kernel == "lasso":
            if isinstance(self.func, list):
                ...
                for i in range(len(self.func)):
                    LoadReconsCoeff.append(self.func[i].predict(xTest[:, self.nonzeroIds[i]]))
