...
def Create(settings, solver_wrapper, solY):
    ...
    return QuadraticDerivativePredictor(settings, solver_wrapper, solY)

class QuadraticDerivativePredictor(CoSimulationPredictor):
    def __init__(self, settings, solver_wrapper, solY):
        ...
        self.interface_dataYvel = solY.GetInterfaceData("velocity")
        self.interface_dataYaccel = solY.GetInterfaceData("acceleration")
    ...
    def Predict(self):
        if self.currentT >= self.launch_time and self.currentT < self.end_time:
            ...
            current_velocity = self.interface_dataYvel.GetData(0)
            current_accel = self.interface_dataYaccel.GetData(0)
